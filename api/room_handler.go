...
func (r *RoomHandler) HandleBookRoom(c *fiber.Ctx) error {
	...
	insertedType, err := r.store.Booking.InsertBooking(c.Context(), &booking)
	if err != nil {
		return err
	}
	return c.Status(http.StatusCreated).JSON(insertedType)
}
...
func (r *RoomHandler) isRoomAvailableForBooking(ctx context.Context, roomID primitive.ObjectID, params BookRoomParams) (bool, error) {
	...
	bookings, err := r.store.Booking.GetBookings(ctx, where)
	if err != nil {
		return false, err
	}
	ok := len(bookings) == 0
